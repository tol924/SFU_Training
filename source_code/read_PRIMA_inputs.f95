module PRIMA_inputs
    ...
    subroutine read_PRIMA_inputs(ascii_data,error_thr,errord_thr,vol_thr,rain_flag,ncol,nrow,ntot,inactive_cells,&
        ...
        integer:: isum,j,inode,niter,i,ndays,ini_dep_flag,freq_err
        real*8:: xllcorner,yllcorner
        real, ALLOCATABLE :: CA_outflow(:)
        integer,allocatable::ind_node(:,:),rvr_cell_ind(:)
        real,allocatable::mann(:)!,vel(:),dt(:),wdepth(:)
...
!        common /dims/ncol,nrow,ntot
       ...
       !!!!start ranking
        ...
        !read river cells index from text file generated by matlab
            ...
            !get the lowest cell on the watershed boundary
            call get_outflow_ind(dem,ind_node,no_data,outflow_cell)
            n_rvr_cell=1
            allocate(rvr_cell_ind(n_rvr_cell))
            rvr_cell_ind=outflow_cell

        end if
        !!!!!initialize variables
        allocate(mann(ntot),wl_lin(ntot)) !vel(ntot),dt(ntot),wdepth(ntot)
        mann=mann1; !manning's coeffecient


        if(ini_dep_flag .eq. 0)then
        ...
        !get index surrounding 8
        allocate(ind_sur(ntot,10))
        !array in_sur cell_linear id of current cell, no_of surrounding cells, surrounding cells linear_id as (1s,2se,3e,4ne,5n,6nw,7w,8sw)
        call neighbor_cells8(ind_node,dem_lin,ind_sur,no_data)
        !write(*,*) ind_sur(50,:)
        allocate(CA_outflow(ndays)) !outflow volume
        CA_outflow=0
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    subroutine get_outflow_ind(dem,ind_node,no_data,outflow_cell)
        integer,intent(in):: ind_node(nrow,ncol)
        real*8,intent(in):: dem(nrow,ncol)
        real*8:: dem_dum(nrow,ncol)
        real,intent(in):: no_data
        integer::ewns_ind(4) !linear index of the lowest point in elevation in the four edges of the watershed (East, West, North, and South
        real::ewns_val(4) !values of lowest cells corresponds to indexes in ewns_ind
        integer:: nrow,ncol,ntot,arminloc
        integer,intent(out)::outflow_cell
        !real::no_data
        common /dims/ncol,nrow,ntot
        write(*,*) 'Getting outflow cell index as the lowest point on the watershed perimeter'
        dem_dum= abs(dem)

        !loop on the first few column (from the left) to get the lowest on the east side
        do i=1,ncol
            armin=MINVAL(dem_dum(:,i))
            if(armin .ne. abs(no_data))then
                arminloc=minloc(dem_dum(:,i),1)
                ewns_ind(1)=ind_node(minloc(dem_dum(:,i),1),i) !east lowest elevation cell index
                ewns_val(1)=armin !east lowest elevation
                exit
            end if
        end do

        !loop on the last few column (from the right) to get the lowest on the west side
        do i=ncol,1,-1
            armin=MINVAL(dem_dum(:,i))
            if(armin .ne. abs(no_data))then
                arminloc=minloc(dem_dum(:,i),1)
                ewns_ind(2)=ind_node(minloc(dem_dum(:,i),1),i) !west lowest elevation cell index
                ewns_val(2)=armin !west lowest elevation
                exit
            end if
        end do

        !loop on the first few rows (from the top) to get the lowest on the north side
        do i=1,nrow
            armin=MINVAL(dem_dum(i,:))
            if(armin .ne. abs(no_data))then
                arminloc=minloc(dem_dum(i,:),1)
                ewns_ind(3)=ind_node(i,minloc(dem_dum(i,:),1)) !north lowest elevation cell index
                ewns_val(3)=armin !north lowest elevation
                exit
            end if
        end do

        !loop on the last few rows (from the bottom) to get the lowest on the south side
        do i=nrow,1,-1
            armin=MINVAL(dem_dum(i,:))
            if(armin .ne. abs(no_data))then
                arminloc=minloc(dem_dum(i,:),1)
                ewns_ind(4)=ind_node(i,minloc(dem_dum(i,:),1)) !south lowest elevation cell index
                ewns_val(4)=armin !south lowest elevation
                exit
            end if
        end do


        min_elev_loc=minloc(ewns_val,1)
        outflow_cell=ewns_ind(min_elev_loc)
        !write(*,*)ewns_val(min_elev_loc)


        return
    end subroutine get_outflow_ind
